...
Calculater::Calculater(bool first_calcul) : Calculater() {
    this->first_calculate = first_calcul;
}

//呼叫運算式分解並計算
bool Calculater::string_calculate(std::string &user_input, c_type &result) {
    Special special;
...
//呼叫運算元做計算
c_type Calculater::op_calculate(char calcul, const c_type &front, const c_type &back) {
    c_type res = 0;
    switch (calcul) {
        case '+':
            res = front + back;
...
//檢查是否須離開
bool Calculater::is_exit() {
    return this->exit;
}

//以下為private成員函式
//檢查有無錯誤
bool Calculater::is_wrong(const std::string &expr) {
    ...
    for (unsigned i = 0; i < expr.size(); i++) {
        ...
        else if (expr[i] == '/' || expr[i] == '%') {    //是否為除法(取餘數)
            unsigned n = i + 1;
    ...
    this->wrong_absolute = (abs % 2 == 0)? false: true; //絕對值成雙，所以對2取餘數應為0
    ...
    //回傳是否有錯誤(只要一個有就不該計算)
    return divide_zero || this->wrong_brackets || this->wrong_absolute || loss_something || not_expression || not_operation || first_calculate_want_ans;
}

//錯誤訊息
void Calculater::warning_message() {
    if (this->divide_zero) {
...
//是否為擁有的運算子(char 型態)
bool Calculater::is_operator(char c) {
    //從ops裡找，如果有就對了
    return (this->ops.find(c) != this->ops.end())? true: false;
}

//是否為擁有的運算子(std::string 型態)
bool Calculater::is_operator(const std::string &op) {
    //如op長度只有1且是運算子，對拉
    return (op.size() == 1 && this->is_operator(op[0]))? true: false;
}

//查看運算子的優先度
int Calculater::pripority(char c) {
    //優先度為ops裡的value值，若沒有運算子，則為最低階運算(不太會發生了)
    return (this->is_operator(c))? this->ops[c] : -1;
}

//檢查是否為數字
bool Calculater::is_number(char c) {
    return (c >= '0' && c <= '9')? true: false;
}

//檢查是否為數字(有正負數的情況)
bool Calculater::is_integer(char op, char front, char back) {
    if (op == '-' || op == '+') {   //看op是否為+,-
...
//檢查是否為空白(space與tab)
bool Calculater::is_space(char c) {
    return (c == '\t' || c == ' ')? true: false;
}

//把運算式的所有字母轉為小寫
void Calculater::to_lower(std::string &s) {
...
//合併不需要的括號與正負號
void Calculater::combine(std::string &expr) {
    static std::string front, back;    //暫存用的字串
    for (unsigned i = 0; i < expr.size(); i++) {
        if (expr[i] == '+' || expr[i] == '-') { //當前為+,-時
            ...
            if (expr[i] == '+' && !is_space(expr[m])) { //當前為+且略過空白後的第二個字元不為空白
                if (expr[n] == '+') {   //當第一個字元為+
                    this->combine_add_sub(expr, front, back, i, n, '+'); //++得+
                    i++;    //跳到第二個字元
                }
                else if (expr[n] == '-') {  //當第一個字元為-
                    this->combine_add_sub(expr, front, back, i, n, '-'); //+-得-
                    i++;    //跳到第二個字元
                }
            }
            else if (expr[i] == '-' && !is_space(expr[m])) {    //當前為-且略過空白後的第二個字元不為空白
                if (expr[n] == '+') {   //當第一個字元為+
                    this->combine_add_sub(expr, front, back, i, n, '-'); //-+得-
                    i++;    //跳到第二個字元
                }
...
//合併不需要的正負號
void Calculater::combine_add_sub(std::string &expr, std::string &front, std::string &back, int before, int after, char op) {
...
//把輸入的運算式拆解成後綴式
unsigned Calculater::infix_to_postfix(std::vector<std::string> &result, const std::string &expr, const c_type &ans, const unsigned start, const char input_op) {
    std::stack<char> op;    //利用stack儲存運算子
    unsigned i;
    for (i = start; i < expr.size(); i++) {
        //如果是數字或(整數且(op不為空或非(result不為空且result最後一個長度為1且result最後一個為'|')))，那就為一整數
        if (this->is_number(expr[i]) || (this->is_integer(expr[i], (i > 0)? expr[i-1]: '\0', expr[i+1]) && (!op.empty() || !(!result.empty() && result.back().size() == 1 && result.back()[0] == '|')))) {
            std::string num("");
            do {
            ...
            if (input_op == '|') {  //input_op為絕對值的話
                ...
                while (input_op == '|' && n < i && this->is_space(expr[i])) {
                    //當input_op為絕對值且n小於i且中間為空白，n加一
                    n++;
                }
...
*/
    ...
    for (std::string expr : exprs) {    //輪過每個exprs的元素
        if (this->is_operator(expr) && (this->operand[expr[0]] == 2)) {
            //expr為運算子且該需要的運算元為2
            c_type back = numbers.top();    //取出最上面的數儲存
            numbers.pop();  //抽出
            c_type front = numbers.top();   //再取一次
            numbers.pop();  //抽出
            ...
            //expr為運算子且該需要的運算元為1
            c_type num = numbers.top(); //取出最上面的數儲存
            numbers.pop();  //抽出
...
//計算階乘
c_type Calculater::factorial(const c_type &num) {
...
//計算指數(只有正整數的次方數)
c_type Calculater::power(const c_type &base, const c_type &times) {
    c_type res = 1;
    if (base < 0) return 0; //負數為分數，以整數來說無法計算，故回傳0
...
//是否為特殊字句
bool Calculater::is_special(const std::string &input, Special &s) {
    bool special = false;
...
//印出特殊字句的特殊輸出
void Calculater::print_special(const Special &s) {
    switch(s) {
        case Shang_xiang:
...
//以毫秒為單位的延遲
void Calculater::m_delay(int time) {
